...
#### Tasks
0. **Initialize:* ```my_calculator = Calculator.new(1)``` would assign 1 to be the starting result of the new my_calculator instance.
0. **Results:* ```my_calculator.result``` would return the current result - it should be readonly. ```my_calculator.reset(x)``` would reset the calculator to x.
